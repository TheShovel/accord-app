...
(function (Scratch) {
  ...
  class BetterInputSP {
    ...
    allFonts() {
      const custFonts = vm.runtime.fontManager
        ? vm.runtime.fontManager
            .getFonts()
    ...
    updateButtonImages(overlay) {
      ...
      if (text) {
        ...
        this.tryOutline(
          text,
          this.mainUIinfo.overlayOutline[0],
          this.mainUIinfo.overlayOutline[1],
        );
      }
      const inputField = overlay.querySelector(
        this.inputType.includes("Single")
          ? "select"
          : this.inputType === "Text Area"
            ? "textarea"
            : "input",
      );
      if (inputField) {
        ...
        inputField.style.width =
          this.inputType === "Color" || this.inputType.includes("Single")
            ? "100%"
            : this.inputType === "Horizontal Slider"
              ? "95%"
              : isNaN(inpWidth) || this.inputType.includes("Dropdown")
                ? "auto"
                : `${inpWidth - 10}px`;
        ...
        inputField.style.accentColor = this.inputFieldColor;
      ...
      if (dropBtn) {
        ...
        dropBtn.style[
          this.dropdwnBtnColor[0].includes("gradient")
            ? "backgroundImage"
            : "backgroundColor"
        ] = this.dropdwnBtnColor[0];
    ...
    setColorSettings(args) {
      ...
      const colorTypeMap = {
        ...
        "Dropdown Button": () => {
          this.dropdwnBtnColor[0] = colorValue;
          this.overlayImage[2] = " ";
        },
    ...
    setInputType(args) {
      if (args.ACTION === "Text" || args.ACTION === "None")
        this.inputType = args.ACTION === "Text" ? "Enabled" : "Disabled";
    ...
    resetInput() {
      this.userInput = this.askBoxInfo[1] > 1 ? [] : "";
    }
      ...
      if (this.askBoxInfo[0] < this.askBoxInfo[1]) {
        ...
        return new Promise((resolve) => {
          ...
          for (const buttonName in this.buttonJSON) {
            ...
            if (btnInfo.name.includes("<newline>"))
              btnContain.appendChild(document.createElement("br"));
            else {
              ...
              btn.addEventListener("click", () => {
                ...
                setInpValue(
                  this.inputType === "Disabled" ? btnInfo.name : this.userInput,
                );
                this.closeOverlay(overlay, hasDecreased);
                hasDecreased = true;
                resolve();
          ...
          if (this.inputType.includes("Dropdown")) {
            ...
            if (this.inputType === "Single Dropdown") {
              ...
              this.optionList.forEach((label, index) => {
                ...
                optRadio.type =
                  this.inputType === "Dropdown" ? "radio" : "checkbox";
                ...
                optRadio.addEventListener("click", () => {
                  if (isMulti) {
                    if (selectOpts.includes(label))
                      selectOpts = selectOpts.filter((item) => item !== label);
                    else selectOpts.push(label);
                    inputField.value =
                      selectOpts.length > 0 ? JSON.stringify(selectOpts) : "";
                  } else {
              ...
              dropdwnBtn.addEventListener("click", () => {
                ...
                dropdwnCont.style.display = this.isDropdownOpen
                  ? "none"
                  : "block";
            ...
            switch (item) {
              ...
              case "input":
                if (this.inputType !== "Disabled") {
                  ...
                  if (this.inputType === "Single Dropdown")
                    overlay.append(dropdwnBtn, createBr());
                  else if (this.inputType.includes("Dropdown"))
                    overlay.append(dropdwnBtn, dropdwnCont, createBr());
                  else if (this.inputType.includes("Slider"))
                    overlay.append(sliderContain, valTxt, createBr());
                  else {
                    setInpValue(this.defaultValue);
                ...
                break;
              case "buttons": {
                overlay.appendChild(btnContain);
                break;
              }
            }
          }
    ...
    setMaxBoxCount(args) {
      ...
      if (this.askBoxInfo[1] > 1 && !Array.isArray(this.userInput))
        this.userInput = [this.userInput];
    }

    setTimeout(args) {
      this.Timeout = Scratch.Cast.toNumber(args.TIME);
    }
    ...
    getUserInput() {
      if (this.askBoxInfo[1] > 1)
        return this.userInput === null ? "[]" : JSON.stringify(this.userInput);
    ...
    setGradient(t) {
      if ("Input Box" === t.COLOR_TYPE)
        throw Error("As of Better Input V4, this Option no Longer Works");
      let i = this.findGradientType(t.COLOR_TYPE),
        e = `linear-gradient(${t.DIR - 90}deg, ${t.COLOR2}, ${t.COLOR1})`;
      "button" !== i[0] ? (this[i][0] = e) : (this.buttonJSON[i[1]].color = e),
        this.activeOverlays.forEach((t) => {
    ...
    setCircleGradient(t) {
      let i = this.findGradientType(t.COLOR_TYPE),
        e = [t.X + 50, t.Y + 50],
        n = `radial-gradient(circle at ${e[0]}% ${e[1]}%, ${t.COLOR2}, ${t.COLOR1})`;
      "button" !== i[0] ? (this[i][0] = n) : (this.buttonJSON[i[1]].color = n),
        this.activeOverlays.forEach((t) => {
          this.updateOverlay(t);
        });
    }
